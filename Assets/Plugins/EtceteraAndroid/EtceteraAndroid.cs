...
public class EtceteraAndroid
{
    ...
    // Volume for speech. 1.0 is full volume.
    public static void setVolume( float volume )
    {
        if( Application.platform != RuntimePlatform.Android )
            return;
        
        _plugin.Call( "setVolume", volume );
    }

    // Set Language for text to speech, not sure how this works yet for droid.
    public static void setLanguage( string language )
    {
        if( Application.platform != RuntimePlatform.Android )
            return;
        
        _plugin.Call( "setLanguage", language );
    }
