...
Opponent.cs contains information about opponent
...
public class Opponent : MonoBehaviour {
	...
	float rotation;
	const float khitdistrange = 1.3f;
	const float kjusthitd = 1.0f;
	const float kspeed = 6.5f;
	const float kturn = 6.0f;
	const float kforce = 12.0f;
	const float kmove = 5.0f;
	const float krange = 0.2f;

	const float kup = 8.5f;

	void SetSkill(float mx)
    {
	    speed = kspeed * mx;
	    turnSpeed = kturn * mx;
	    moveRange = kmove * mx;
		hitDist = khitdistrange * mx;
		justHitd = kjusthitd * (1/mx);
		hitDistRange = krange * mx;
		//force = kforce;// + (0.5f * mx);
		//upForce = kup;
	}

	public void SetDifficulty(float factor)
	{
		SetSkill(factor);
		skill = factor;
	}
	...
	public IEnumerator JustHit()
    {
		...
		yield return new WaitForSeconds(justHitd);
